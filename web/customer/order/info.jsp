...
<c:if test="${!empty order}">
    <div>
        ...
        <p>订单状态：<span
        class="text-danger">${order.status.name}</span></p>
        <p>
        <c:if test="${order.status==OrderStatus.UNPAID}">
            <a href="order/pay?orderId=${order.orderId}">付款</a>
            <a href="order/cancel?orderId=${order.orderId}">取消订
                单</a>
        </c:if>
        ...
        <c:if test="${order.status==OrderStatus.SHIPPED}">
            ...
            <a href="order/finish?orderId=${order.orderId}">确认收
                货</a>
        </c:if>
        <c:if test="${order.status==OrderStatus.CANCEL}">
            <a href="order/del?orderId=${order.orderId}">删除订单
            </a>
        </c:if></p>
        <address>
    ...
    <c:forEach items="${order.bookItems}" var="item"
        ...
        alt="图书封面"></a>
            ...
            <td>
                <a href="book/info?id=${item.book.id}">${item.book.ti tle} </a>
            </td>
            <td>${item.num} </td>
